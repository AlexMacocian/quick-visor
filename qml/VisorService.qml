...
QtObject {
    ...
    // Hyprland 0.55+ configs are Lua. `hyprctl keyword` is refused against them
    // ("keyword can't work with non-legacy parsers. Use eval.") and the legacy
    // string form of `hyprctl dispatch` is parsed as Lua, so both go through
    // `hyprctl eval` with explicit hl.* calls instead.
    function luaQuote(value) {
        return '"' + String(value).replace(/\\/g, "\\\\").replace(/"/g, '\\"') + '"';
    }

    function evalCommand(lua) {
        return "hyprctl eval " + shQuote(lua);
    }

    readonly property string monitorLuaPath: "${XDG_CONFIG_HOME:-$HOME/.config}/hypr/monitors.lua"

    function monitorSpec(m) {
        if (monitorEnabled(m)) {
            return "hl.monitor({ output = " + luaQuote(m.name)
                + ", mode = " + luaQuote(modeForHyprland(monitorMode(m)))
                + ", position = " + luaQuote(monitorX(m) + "x" + monitorY(m))
                + ", scale = " + monitorScale(m) + " })";
        }
        return "hl.monitor({ output = " + luaQuote(m.name) + ", disabled = true })";
    }
    ...
    function applyLayout() {
        ...
        for (let i = 0; i < monitors.length; i++) {
            const m = monitors[i];
            const command = evalCommand(monitorSpec(m));
            if (monitorEnabled(m)) {
                ...
                dpmsCommands.push(evalCommand("hl.dispatch(hl.dsp.dpms({ action = \"on\", monitor = "
                    + luaQuote(m.name) + " }))"));
            } else {
        ...
        // Persist to the Hyprland-required monitors.lua, but only if it already exists.
        const persist = "CONF=\"" + monitorLuaPath + "\"; "
            + "if [ -f \"$CONF\" ]; then printf '%s' " + shQuote(monitorLuaContent()) + " > \"$CONF\"; fi";
